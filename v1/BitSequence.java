 ...
 * serialized.** @author Nathan Sprague* @version V1.1 12/6/16
 **/
public class BitSequence implements Iterable<Integer>, Serializable {
    ...
    public BitSequence(String bitString) {
    	this();
    	this.appendBits(bitString);
    }

    public BitSequence(byte b) {
    	this();
    	String s2 = String.format("%8s", Integer.toBinaryString(b & 0xFF))
				.replace(' ', '0');
    	this.appendBits(s2);
	}

	public BitSequence(short length) {
		this();
		this.appendBits(Integer.toBinaryString(0xFFFF & length));
	}
    ...
    /**
     * Append a bit to the end of this sequence. 0 will be interpreted as the
     * bit* 0, all other values will be interpreted as the bit 1.
     */
    ...
    /***
     * Set the value of a single bit at a specified index. 0 will be
     * interpreted* as the bit 0, all other values will be interpreted as the
     * bit 1.
     */
    ...
    public boolean equals(Object other) {
        ...
        if (otherSeq.length() != length()) {
          ...
          // index.for (int i = 0; i < length() / 8 + 1; i++) {if (bytes[i] !=
          // otherSeq.bytes[i]) {return false;}}
        return true;
    }
    ...
    public byte toByte() throws LengthException {
    	
    	// Throws this exception if the current BitSequence cannot 
    	// be turned into a char.
    	if( this.length() != 8 ) {
    		throw new LengthException("Current BitSequence cannot be "
    				+ "turned into a byte.");
    	}
    	
    	// get the byte value of the sequence
    	int total = 0;
    	total += ((Math.pow(2, 7)) * getBit(0));
    	total += ((Math.pow(2, 6)) * getBit(1));
    	total += ((Math.pow(2, 5)) * getBit(2));
    	total += ((Math.pow(2, 4)) * getBit(3));
    	total += ((Math.pow(2, 3)) * getBit(4));
    	total += ((Math.pow(2, 2)) * getBit(5));
    	total += ((Math.pow(2, 1)) * getBit(6));
    	total += ((Math.pow(2, 0)) * getBit(7));
    ...
    /***
     * This method overrides the default writeObject method to make sure that
     * the* array is trimmed before serialization.** @throws IOException If
     * there is a problem writing to the provided output* stream.
     */
